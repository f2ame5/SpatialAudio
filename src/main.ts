...
export class Main {
  ...
  private setupDebugUI(): void {
    ...
    wallsFolder
      .add(this.roomConfig.materials.walls, "absorption125Hz", 0, 1)
      .name("125Hz Absorption");
    wallsFolder
      .add(this.roomConfig.materials.walls, "absorption1kHz", 0, 1)
      .name("1kHz Absorption");
    wallsFolder
      .add(this.roomConfig.materials.walls, "absorption8kHz", 0, 1)
      .name("8kHz Absorption");

    // Add frequency-dependent absorption controls for ceiling
    const ceilingFolder = materialsFolder.addFolder("Ceiling");
    ceilingFolder
      .add(this.roomConfig.materials.ceiling, "absorption125Hz", 0, 1)
      .name("125Hz Absorption");
    ceilingFolder
      .add(this.roomConfig.materials.ceiling, "absorption1kHz", 0, 1)
      .name("1kHz Absorption");
    ceilingFolder
      .add(this.roomConfig.materials.ceiling, "absorption8kHz", 0, 1)
      .name("8kHz Absorption");

    // Add frequency-dependent absorption controls for floor
    const floorFolder = materialsFolder.addFolder("Floor");
    floorFolder
      .add(this.roomConfig.materials.floor, "absorption125Hz", 0, 1)
      .name("125Hz Absorption");
    floorFolder
      .add(this.roomConfig.materials.floor, "absorption1kHz", 0, 1)
      .name("1kHz Absorption");
    floorFolder
      .add(this.roomConfig.materials.floor, "absorption8kHz", 0, 1)
      .name("8kHz Absorption");
    ...
    this.sourceControllers = {
      ...
      y: sourceFolder
        .add(sourcePosition, "y", 0, this.roomConfig.dimensions.height)
        .onChange((value: number) => {
          const pos = this.sphere.getPosition();
          pos[1] = value;
          this.sphere.setPosition(pos);
    ...
    sourceFolder
      .add(this.sourceParams, "sourcePower", -60, 20)
      .name("Power (dB)")
      .onChange((value: number) => {
        this.sourceParams = {
          ...this.sourceParams,
    ...
    const audioControls = {
      ...
      play: async () => {
        ...
        try {
          ...
          this.currentAudioBuffer = await this.audioProcessor.loadAudioFile(
            fileUrl
          );
    ...
    audioFolder
      .add(audioControls, "selectedFile", Object.keys(this.soundFiles))
      .name("Sound File");
