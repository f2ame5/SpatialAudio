...
export class DiffuseFieldModelModified {
  ...
  constructor(sampleRate: number, roomConfig: any) {
    ...
    console.log(
      `[DFM CONSTRUCTOR] Initial Room Vol: ${this.roomVolume.toFixed(
        2
      )}, Surface Area: ${this.surfaceArea.toFixed(2)}`
    );
    console.log(
      `[DFM CONSTRUCTOR] Initial Mean Absorption:`,
      JSON.parse(JSON.stringify(this.meanAbsorption))
    );
  }
  ...
  public generateDiffuseField(
    ...
    console.log(
      `[DFM generateDiffuseField] Target duration: ${safeDuration.toFixed(2)}s`
    );

    for (const freq of frequencies) {
      const sampleCount = Math.max(
        Math.ceil(safeDuration * this.sampleRate),
        512
      );
      ...
      const echoDensity = Math.max(
        500,
        1000 * (safeRoomVolume / 100) * (1 + diffusion)
      );
      ...
      console.log(
        `[DFM generateDiffuseField] Freq: ${freq}Hz, RT60: ${rt60.toFixed(
          2
        )}s, BufferLen: ${sampleCount}, EchoDensity: ${echoDensity.toFixed(0)}`
      );
      this.generateVelvetNoise(
        buffer,
        rt60,
        echoDensity,
        meanTimeGap,
      ...
      console.log(
        `[DFM generateDiffuseField] Freq: ${freq}Hz, Generated Velvet Noise RMS: ${calculateRMS(
          buffer
        ).toExponential(3)}`
      );
  ...
  private generateVelvetNoise(
    ...
    // Scale pulse amplitude to compensate for echo density.
    // As density increases (e.g., in a larger room), each pulse must be quieter
    // to maintain a consistent energy level, which prevents the reverb from
    // getting louder in bigger spaces. The '5000' is an empirical tuning
    // factor for a baseline density.
    const energyCorrectionFactor = Math.sqrt(5000 / safeEchoDensity);
    ...
    for (let i = 0; i < totalPulses; i++) {
      ...
      const jitter =
        ((Math.random() - 0.5) * 2 * diffusion * buffer.length) / totalPulses;
      ...
      const decayAmplitude = Math.exp(decayTerm);

      // Apply the correction factor to the natural decay
      const amplitude = decayAmplitude * energyCorrectionFactor;
  ...
  public applyFrequencyFiltering(
    ...
    const maxEnergy = Math.max(
      ...Object.values(totalEnergyPerBand).filter((e) => e > 0)
    );
    if (maxEnergy > 0) {
      for (const freq of frequencies) {
    ...
    console.log(
      "[DFM applyFrequencyFiltering] Data-driven Band Gains:",
      bandGains
    );
    ...
    const reverbLevelScalar = 0.5;
    const overallGain = Math.sqrt(maxEnergy) * reverbLevelScalar;

    if (overallGain > 0) {
      for (let i = 0; i < outputIR.length; i++) {
        outputIR[i] *= overallGain;
      }
    }
  ...
  public processLateReverberation(
    ...
    this.meanAbsorption = this.calculateMeanAbsorption(roomConfig.materials);
    console.log(
      `[DFM processLateReverberation] Updated Room Vol: ${this.roomVolume.toFixed(
        2
      )}, Surface Area: ${this.surfaceArea.toFixed(2)}`
    );
    console.log(
      `[DFM processLateReverberation] Updated Mean Absorption:`,
      JSON.parse(JSON.stringify(this.meanAbsorption))
    );
    ...
    if (!lateHits || lateHits.length === 0) {
      ...
      console.warn(
        "[DFM processLateReverberation] No late hits, returning default IR."
      );
      return [defaultIR.slice(), defaultIR.slice()];
    }

    const rt60Values = this.calculateRT60Values(lateHits, roomConfig);
    const rt60NumericValues = Object.values(rt60Values).filter(
      (val) => typeof val === "number" && isFinite(val)
    );
    const maxRT60 =
      rt60NumericValues.length > 0 ? Math.max(...rt60NumericValues) : 1.0;
    const reverbGenerationDuration = Math.min(
      2.5,
      Math.max(maxRT60 * 1.2 + 0.1, 0.3)
    );
    console.log(
      `[DFM processLateReverberation] Max RT60: ${maxRT60.toFixed(
        2
      )}s, Reverb Gen Duration: ${reverbGenerationDuration.toFixed(2)}s`
    );

    const diffuseResponses = this.generateDiffuseField(
      reverbGenerationDuration,
      rt60Values
    );
    ...
    if (!monoIR || monoIR.length === 0) {
      ...
      defaultIR[0] = 0.01;
      console.warn(
        "[DFM processLateReverberation] Mono IR is empty after filtering, returning default."
      );
      return [defaultIR.slice(), defaultIR.slice()];
    }
    ...
    if (!hasContent) {
      monoIR[0] = 0.01;
      console.warn(
        "[DFM processLateReverberation] Mono IR has no significant content, added impulse."
      );
    }
    ...
    for (let i = 0; i < monoIR.length; i++) {
      ...
      const allPassOutputL =
        allPassG_L * filteredSampleL +
        x_n_minus_D_L -
      ...
      const allPassOutputR =
        allPassG_R * filteredDelayedSample +
        x_n_minus_D_R -
    ...
    console.log(
      `[DFM processLateReverberation] Final stereo late IR generated. Left RMS: ${calculateRMS(
        leftIR
      ).toExponential(3)}, Right RMS: ${calculateRMS(rightIR).toExponential(3)}`
    );
    return [leftIR, rightIR];
  }

  private calculateRT60Values(
    lateHits: any[],
    roomConfig: any
  ): { [freq: string]: number } {
    const frequencies = [
    ...
    for (const freq of frequencies) {
      ...
      const timeBins = new Array(numBins)
        .fill(0)
        .map((_, i) => (i + 0.5) * binDuration);
    ...
    console.log(
      "[DFM calculateRT60Values] Calculated RT60s from simulation:",
      rt60Values
    );
    return rt60Values;
  }
